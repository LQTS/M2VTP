...
def parse_task(env_config, device, n_per_env, multi_envs=False, envs_names=None):
    ...
    if multi_envs:
        env = make_multi_vec_envs(envs_names, env_config, n_per_env, device)
    else:
...
def main(args):
    ...
    env_eval = parse_task(args.task_envs, device=args.device, n_per_env=args.models["learn"]["n_per_env"], multi_envs=True,
                     envs_names=args.task_envs['eval_env_names'])
    logger = DataLog()
    assert os.path.isdir(args.resume_model)

    model_path = os.path.join(args.resume_model, 'checkpoint')

    for i in tqdm(range(0, 3001, 200)):
        args.resume_model = os.path.join(model_path, f'model_{i}.pt')
        logger.log_kv('model', f'model_{i}')
        # set up policy
        sarl = process_sarl(args, env, args.models, args.logger_dir)
        reward, success = sarl.eval(200)
